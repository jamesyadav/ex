...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    print("\n")
    mesgdcrt.SuccessMessage("Bombing completed! Mr james")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 500000, "call": 10000, "mail": 100}
        cc, target = "", ""
        if mode in ["sms", "call"]:
            ...
            while (choice not in avail_choice):
                ...
                for key, value in avail_choice.items():
                    print("[ {key} ] {value} BOMB".format(key=key,
                                                          value=value))
                print()
                choice = input(mesgdcrt.CommandMessage("Enter Choice Mr : "))
